  ...
  if (rows.length === 0) {
    return (
      <p className="py-6 text-sm text-muted">
        No active clients yet. Health is scored once a client goes Active and
        has two full reporting weeks behind it.
      </p>
    );
  }
  return (
      ...
      {rows.map((row) => (
        <li key={row.id}>
          {/* Two lines rather than one: the clinic name and the badge are
              what the card is for, and squeezing the sparkline in beside them
              at this width would truncate both. */}
          <Link
          ...
          >
            <span className="flex items-center gap-2.5">
              ...
              <HealthBadge status={row.health.status} reason={row.health.reason} />
            </span>
