        ...
        <button
          ...
          title={
            !enabled
              ...
              : !runnable
                ? "This stage runs on a qualified candidate with nobody named — that is what stops it spending runs on clinics that would have been rejected"
                : `Runs ${actorId}, anchored to “${clinicName}”`
          }
        ...
        {contactName ? (
          ...
          <div>
            ...
            <p className="mt-1 text-xs leading-relaxed text-muted">
              {searched
                ? "The search ran and turned up nobody who could be verified against this clinic. Nothing has been guessed at — run it again, or type a name into the Contact fields."
                : "Nobody has looked yet. This clinic has qualified on its own evidence; the search finds who to talk to."}
            </p>
          </div>
        )}
      </div>
          ...
          {result.status === "failed" && (
            <div className="rounded-[10px] border border-bad/30 bg-bad-soft/60 px-4 py-3">
              <p className="text-xs leading-relaxed text-muted">
                Every step that could search failed, so nothing was concluded —
                the candidate is untouched. The errors are in the log below;
                fix what they name and run it again.
              </p>
            </div>
          )}
          ...
          {result.status === "found" && result.promotedLeadId && (
            <div className="rounded-[10px] border border-ok/30 bg-ok-soft/60 px-4 py-3">
              <p className="text-sm font-medium">
                Promoted — a decision maker was found, so this clinic went to the
                pipeline on the score it already had.
              </p>
              <Link
...
// The confidence, coloured by what it is claiming. Green only for High, which
// is the one level where the clinic itself said so — an amber Medium and a
// grey Low are the honest colours for "a profile says" and "somebody works
// here".
